...
SimpleAvatar.prototype.ServerInitialize = function() {
    ...
    // Default avatar ref
    var avatarurl = "asian_boy.avatar";
    ...
    if (clientPtr != null)
    {
        ...
        {
            print("Avatar from login parameters enabled: " + avatarurlProp);
...
SimpleAvatar.prototype.ClientInitialize = function() {
    ...
    if (this.me.name == "Avatar" + client.GetConnectionID()) {
        ...
        // Init. underwater post-process effect.
        this.waterEntity = scene.GetEntityByName("water");
        this.waterEntity.CreateLocalComponent("EC_OgreCompositor", "UnderwaterEffect");
        this.waterEntity.ogrecompositor.compositorName = "UnderWater";
        this.waterEntity.ogrecompositor.parameters = ["normalStr=0.02", "speedX=0.2", "speedY=0.20"];
    }
    else
    {
...
SimpleAvatar.prototype.ClientUpdate = function(frametime) {
    ...
    if (this.ownAvatar) {
        ...
        // Enable underwater post-process effect, if under the waterplane.
        if (this.waterEntity)
        {
            var w = this.waterEntity;
            var camUnderwater = w.waterplane.IsCameraInsideWaterCube();
            if (camUnderwater && !w.ogrecompositor.enabled)
                w.ogrecompositor.enabled = true;
            else if (!camUnderwater && w.ogrecompositor.enabled)
                w.ogrecompositor.enabled = false;
        }
    }
